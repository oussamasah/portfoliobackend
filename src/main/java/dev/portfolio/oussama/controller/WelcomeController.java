...
public class WelcomeController {
    ...
    public ResponseEntity<?> checkJobDescriptionCompatibility(@RequestBody String jobDescription) {
        // Extract skills from the job description and calculate compatibility

        Map<String, Object> result = this.welcomeService.extractSkills(jobDescription);
    ...
    public ResponseEntity<?> save(@RequestParam("title") String title,
        ...
        if (cv != null && !cv.isEmpty()) {
            try {
                // Avoid overwriting files with the same name
                String extension = cv.getOriginalFilename().substring(cv.getOriginalFilename().lastIndexOf('.') + 1);
                String cvFilename = Instant.now().getEpochSecond() +"."+  extension;
                if (cvFilename != null) {
